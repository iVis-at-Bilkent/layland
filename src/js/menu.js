...
function evaluate(layoutTime) {
  ...
  document.getElementById("density").innerHTML = parseFloat((cy.edges().length) / ((cy.nodes().length * (cy.nodes().length - 1)) / (2))).toFixed(4);
...
$("body").on("click", "#runLayout", function () {
  ...
  if (layoutType.options[layoutType.selectedIndex].text == "fCoSE") {
    ...
    startTime = performance.now();
    let options = layoutProperties.getCoseProperties();
    console.log(options)
    cy.layout(options).run();
    endTime = performance.now();
    evaluate(endTime - startTime);
    ...
    startTime = performance.now();
    cy.layout({
      name: "cola", flow: { axis: 'y', minSeparation: 100 }, randomize: true, animate: false,
      unconstrIter: 50, userConstIter: 100, allConstIter: 200
    }).run();
    endTime = performance.now();
    evaluate(endTime - startTime);
  }
  else if (layoutType.options[layoutType.selectedIndex].text == "Circle") {
    startTime = performance.now();
    let option = layoutProperties.getCircleProperties();
    cy.layout(option).run();
    endTime = performance.now();
    evaluate(endTime - startTime);
  }

  else if (layoutType.options[layoutType.selectedIndex].text == "Grid") {
    startTime = performance.now();
    let option = layoutProperties.getGridProperties();
    cy.layout(option).run();
    endTime = performance.now();
    evaluate(endTime - startTime);
  }
    ...
    startTime = performance.now();
    let option = { name: "random", padding: 20, animate: "end", animationDuration: 1000 };
    cy.layout(option).run();
    endTime = performance.now();
    evaluate(endTime - startTime);
  }
